...
class SevenSegDisplay(object):
    ...
    def main(self, sc):
        ...
        screen_size = [400,200]      # screen size              
        ss = int(screen_size[0]/8)        # segment size - many other parameters are proportional to this value
        sw = ss/10   # segment width
        screen_size[1]=(2*ss)+(2*sw)
        screen_size[0]=screen_size[0]+sw
        digitspace = ss/4 + ss
        ...
        def rotate_segment (segment ,q, sp, segs):   # segment, quadrant of rotation, start point, segments to redraw
            ...
            if q == 1: sa,ea = 0,90
            if q == 2: sa,ea = 90,180
            if q == 3: sa,ea = 180,270
            if q == 4: sa,ea = 270,360
            #anti-clockwize
            if q == -1: sa,ea = 90,0
            if q == -2: sa,ea = 180,90
            if q == -3: sa,ea = 270,180
            if q == -4: sa,ea = 360,270
...
if __name__ == '__main__':
    pygame.init()
    sc = pygame.display.set_mode((1024,768),RESIZABLE,NOFRAME)
